...
def save_as(files, codes)
	codes = codes.strip.split(/\<\s*\/\s*[Ee][Nn][Dd]\s*\>/)
...
def test(*arguments, codes)
	codes.strip.split(/\<\s*\/\s*[Ee][Nn][Dd]\s*\>/).map
		.with_index { |code, index| "-> Code #{index.next}:\n\n#{evaluate(code)}\n#{'-' * 50}" }
end
	...
	begin
		__v__ = code.to_s.strip.each_line.map do |__l__|
			...
			__l__.strip.then do |__e__|
				[__e__].<<(__e__.empty? || __e__[0] == '#' ? '' : "# => #{__b__.eval(__e__).then { |v| v.nil? ? 'nil' : v } }")
			end
		end
		...
		Kernel.warn code.each_line.to_a[0..__inx__ - 1].map
